	...
	public function index()
	{
		...
		if ($this->group_id=='3' || $this->group_id=='4'){
			...
			foreach ($data['label'] as $key => $value) {
				$data['labels'][]='rencana '.$value->kode_komponen;
				$data['labels'][]='realisasi '.$value->kode_komponen;
				$a=$this->Realisasi_model->chart($this->user->id_unit,$value->kode_komponen);
				$b=$this->Realisasi_model->chart($this->user->id_unit,$value->kode_komponen);
				...
				$i++;
				// $color = '#' . $this->generate_hex_color(mt_rand(0,255)) . $this->generate_hex_color(mt_rand(0,255)) . $this->generate_hex_color(mt_rand(0,255));
				// $data['color'][]=$color;
				// $data['color'][]=$color;
			}
			...
			$data['chart']=$this->Sub_komponen_model->chart($this->user->id_unit);
			$data['title'] = 'Realisasi Capaian Tahun '.$this->tahun;
		}else{
			$data['total_pagu']=$this->Kegiatan_model->sum_by_jenis_unit($this->user->id_unit)->jumlah;
			$data['total_realisasi_capaian']=$this->Kegiatan_model->sum_by_jenis_unit($this->user->id_unit)->capaian;
			$data['total_rencana_capaian']=$this->Kegiatan_model->sum_by_jenis_unit($this->user->id_unit)->rencana_capaian;
			$data['chart']=$this->Kegiatan_model->chart();
			$data['title'] = 'Realisasi Capaian Program Dukungan Manajemen dan Pelaksanaan Tugas Teknis Lainnya Kementerian Riset, Teknologi dan Pendidikan Tinggi';
		}
		$data['subtitle'] = '';
        $data['crumb'] = [
            'Dashboard' => '',
