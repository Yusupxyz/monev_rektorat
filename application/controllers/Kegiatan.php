...
class Kegiatan extends CI_Controller
{
    ...
    public function index()
    {
        ...
        for ($i=0;$i<$this->Kegiatan_model->count_kegiatan()->count;$i++){
            $count_child[] =$this->Kegiatan_model->count_child($i);
            $komponen[] = $this->Komponen_model->get_by_id_kegiatan($i);
        }

        for ($i=0;$i<$this->Komponen_model->count_komponen()->count;$i++){
            $count_child_komponen[] =$this->Komponen_model->count_child($i);
            $subkomponen[] = $this->Sub_komponen_model->get_by_id_komponen($i);
        }
    ...
    public function create($page=null,$id=null) 
    {
        ...
        if ($page=='program'){
            ...
            $data['action'] =site_url('kegiatan/create_action_komponen/'.$id);
    ...
    public function create_action_subkomponen($id=null) 
    {
        ...
        if(! $this->Sub_komponen_model->is_exist($this->input->post('id_subkomponen'))){
            ...
            //update komponen
            $id_komponen=$this->Komponen_model->get_id_komponen($user->id_unit);
            ...
            //update sub output
            $id_kegiatan=$this->Komponen_model->get_id_kegiatan($user->id_unit);
            ...
            for ($i=4;$i>1;$i--){
                $sum = array(
                    'jumlah' => $this->Kegiatan_model->sum($i,$user->id_unit)->sum
                );
                $id_kegiatan= $this->Kegiatan_model->get_id_kegiatan($i-1,$user->id_unit)->id_kegiatan;
