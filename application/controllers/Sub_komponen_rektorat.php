...
class Sub_komponen_rektorat extends CI_Controller
{
    ...
    public function index()
    {
        ...
        if ($q <> '') {
            $config['base_url'] = base_url() . 'sub_komponen_rektorat?q=' . urlencode($q);
            $config['first_url'] = base_url() . 'sub_komponen_rektorat?q=' . urlencode($q);
        } else {
            $config['base_url'] = base_url() . 'sub_komponen_rektorat';
            $config['first_url'] = base_url() . 'sub_komponen_rektorat';
        }

        $config['per_page'] = 10;
        $config['page_query_string'] = TRUE;
        $config['total_rows'] = $this->Sub_komponen_rektorat_model->total_rows($q);
        $sub_komponen_rektorat = $this->Sub_komponen_rektorat_model->get_limit_data($config['per_page'], $start, $q);

        $this->load->library('pagination');
        $this->pagination->initialize($config);

        $data = array(
            'sub_komponen_rektorat_data' => $sub_komponen_rektorat,
            'q' => $q,
            'pagination' => $this->pagination->create_links(),
            'total_rows' => $config['total_rows'],
            'start' => $start,
        );
        $data['title'] = 'Sub Komponen Rektorat';
        $data['subtitle'] = '';
        $data['crumb'] = [
            'Sub Komponen Rektorat' => '',
        ];
        $data['code_js'] = 'sub_komponen_rektorat/codejs';
        $data['page'] = 'sub_komponen_rektorat/Sub_komponen_rektorat_list';
        $this->load->view('template/backend', $data);
    }

    public function read($id) 
    {
        $row = $this->Sub_komponen_rektorat_model->get_by_id($id);
        if ($row) {
            $data = array(
        ...
        $data['page'] = 'sub_komponen_rektorat/Sub_komponen_rektorat_read';
        $this->load->view('template/backend', $data);
        } else {
            $this->session->set_flashdata('message', 'Record Not Found');
            redirect(site_url('sub_komponen_rektorat'));
        }
    }
    ...
    public function create() 
    {
        $data = array(
            'button' => 'Create',
            'action' => site_url('sub_komponen_rektorat/create_action'),
        ...
        $data['page'] = 'sub_komponen_rektorat/Sub_komponen_rektorat_form';
        $this->load->view('template/backend', $data);
    }
...
if(! $this->Sub_komponen_rektorat_model->is_exist($this->input->post('id_subkomponen'))){
    ...
    public function update($id) 
    {
        ...
        if ($row) {
            $data = array(
                'button' => 'Update',
                'action' => site_url('sub_komponen_rektorat/update_action'),
    ...
    public function update_action() 
    {
		...
		'jumlah_capaian' => $this->input->post('jumlah_capaian',TRUE),
            ...
            $this->Sub_komponen_rektorat_model->update($this->input->post('id_subkomponen', TRUE), $data);
    ...
    public function delete($id) 
    {
        ...
        if ($row) {
            $this->Sub_komponen_rektorat_model->delete($id);
    ...
    public function deletebulk(){
        $delete = $this->Sub_komponen_rektorat_model->deletebulk();
        if($delete){
            $this->session->set_flashdata('message', 'Delete Record Success');
        }else{
...
/* End of file Sub_komponen_rektorat.php */
/* Location: ./application/controllers/Sub_komponen_rektorat.php */
