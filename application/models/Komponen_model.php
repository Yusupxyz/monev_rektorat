...
class Komponen_model extends CI_Model
{
    ...
    {
        $this->db->select('komponen.id_komponen');
    ...
    function get_by_id_kegiatan($i)
    {
        $result=$this->db->query ('SELECT *  FROM komponen WHERE id_kegiatan = 
                     (select id_kegiatan from kegiatan limit '.$i.',1)')->result();
        return $result;
    }
...
{
    $result=$this->db->query ('SELECT count(*) as "jumlah_anak" FROM `komponen` LEFT join sub_komponen on komponen.id_komponen=sub_komponen.id_komponen 
    ...
    {
        $result=$this->db->query ('SELECT count(*) as "count" from komponen')->row();
        return $result;
    }
